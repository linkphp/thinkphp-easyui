			...
			<tr>
				<td>
					...
					<form id="Core_User_Search_Form" style="float:right">
						...
						<span style="padding-left:10px;">用户：</span><input name="search[branch_id]" class="easyui-combobox" style="width:120px" data-options="valueField: 'id',textField: 'title',url: '<?php echo U('Base/listBranch');?>/type/all'">
						<span style="padding-left:10px;">角色：</span><input name="search[group_id]" class="easyui-combobox" style="width:120px" data-options="valueField: 'id',textField: 'title',url: '<?php echo U('Base/listGroup');?>/type/all'">
...
window.Core_User = {
    ...
    exportExcel: function(){
        location.href="<?php echo U('User/exportExcel');?>";
},
	//编辑用户
	edit: function(id){
		if(id == null){
		...
		$($.Oa.dialog_1).dialog({
			...
			height: '300',
			iconCls: 'icon-edit',
			href: '<?php echo U("User/editUser/");?>/id/'+id,
			modal: true
		});
	},
	...
	delete: function(id){
		...
		$.Oa.confirm('$.Oa.delData("'+id+'","<?php echo U("User/delUser");?>")')
	},
	//修复表
	resetUserPass: function(uid){
		//其实delData只是个传递参数去post的方法,可以用于增删改查
		$.Oa.confirm('$.Oa.delData("'+uid+'","<?php echo U("User/resetUserPass");?>")');
	}
}
</script>
<script>
...
$("#Core_User").datagrid({
	...
	columns:[[
		...
		{field:'name',title:'账号',width:80},
		{field:'title',title:'昵称',width:80},
		...
		{field:'opert_id',title:'操作',width:40,formatter:Core_User.opert}
	]],
	pagination:true,
	pagePosition:'bottom',
	pageNumber:1,
