...
window.Core_Job = {
	...
	order: function(){
		...
		$.post("<?php echo U('Core/sortJob');?>",$(".sort-input-Job").serialize(),function(data){
	...
	edit: function(id){
		...
		$($.Oa.dialog_1).dialog({
			...
			height: '150',
			iconCls: 'icon-edit',
			href: '<?php echo U("Core/editJob/");?>/id/'+id,
			modal: true
		});
	},
	...
	delete: function(id){
		...
		$.Oa.confirm('$.Oa.delData("'+id+'","<?php echo U("Core/delJob");?>")')
	}
}
</script>
<script>
