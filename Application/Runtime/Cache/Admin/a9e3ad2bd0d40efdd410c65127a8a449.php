...
window.Core_Branch = {
	...
	order: function(){
		...
		$.post("<?php echo U('Core/sortBranch');?>",$(".sort-input-Branch").serialize(),function(data){
	...
	add: function(){
		...
		$($.Oa.dialog_1).dialog({
			...
			height: '181',
			iconCls: 'icon-add',
			href: '<?php echo U("Core/addBranch/");?>/branch_id/'+branch_id,
			modal: true
		});
	},
	...
	edit: function(id){
		...
		$($.Oa.dialog_1).dialog({
			...
			height: '181',
			iconCls: 'icon-edit',
			href: '<?php echo U("Core/editBranch/");?>/id/'+id,
			modal: true
		});
	},
	...
	delete: function(){
		...
		id = row.id;
		$.Oa.confirm('$.Oa.delData("'+id+'","<?php echo U("Core/delBranch");?>")')
	}
}
</script>
<script>
